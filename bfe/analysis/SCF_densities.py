        ...
        for j in range(nbins):
            for k in range(nbins):
                ...
                if quantity == "acceleration":
                    almc = biff.gradient(xyz, Slmc, Tlmc, M=1, r_s=rs_lmc, G=G) 
                    amw = biff.gradient(xyz_lmc, Smw, Tmw, M=1, r_s=rs_mw, G=G)
...
if __name__ == "__main__":
    ...
    for i in range(110,111):
        ...
        #y = pos[:,1]
        #z = pos[:,2]
        pos = grid(400, nbins, 3)
        x = pos[0]#.flatten()
        y = pos[1]#.flatten()
        z = pos[2]#.flatten()
        #ns = scf_density_mw(x, y, z, Ss, Ts, rs_mw)
    
        Smw, Tmw, N = coefficients_smoothing.get_coefficients('LMC5', 'isotropic', 'MWLMC', snap=i, sn=5, mass=1.84E-6)
        Slmc, Tlmc, N = coefficients_smoothing.get_coefficients('LMC5','isotropic','LMC', snap=i, sn=8, mass=1.84E-6)
        ...
        #ens_base = scf_density_grid(x, y, z, Smw, Tmw, Slmc, Tlmc, nbins, rs_mw, rs_lmc, lmc_com, quantity, G)
        ...
        density_fname = "rho_mwlmc3_bfe_{}_r_{:0>3d}.txt".format(nbins, i)
        write_density(density_fname, dens_all, x.flatten(), y.flatten(), z.flatten())
